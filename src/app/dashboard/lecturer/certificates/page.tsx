...
// Define RawCertificate type for mapping
...
export default function LecturerCertificatesPage() {
  ...
  async function fetchSessionsList() {
    // Only fetch sessions assigned to this lecturer
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
      setSessionsList([]);
      return;
    }
    const { data: lecturerSessions, error } = await supabase
      .from('lecturer_sessions')
  ...
  return (
    <DashboardLayout role="lecturer">
      <h2 className="text-2xl font-bold text-blue-800 dark:text-blue-200 mb-6">My Certificates</h2>
